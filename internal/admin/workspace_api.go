...
type WorkspaceInfo struct {
	...
	Services     []runtime.McpServiceInfo `json:"services,omitempty"`
}

// handleGetAllWorkspaces 获取所有工作空间
func (h *Handler) handleGetAllWorkspaces(c echo.Context) error {
	xl := xlog.NewLogger("GET-WORKSPACES")
	xl.Info("Get all workspaces")

	// 通过 service manager 获取所有工作空间
	wsMap := h.services.(*workspaces.ServiceManager).GetWorkspaces()

	var workspaceInfos []WorkspaceInfo
	for id, workspace := range wsMap {
		...
		// 获取工作空间的session数量
		sessions := h.services.GetWorkspaceSessions(xl, workspaces.NameArg{
			Workspace: id,
		})
...
func (h *Handler) handleCreateWorkspace(c echo.Context) error {
	...
	_, err := h.services.GetMcpService(xl, workspaces.NameArg{
		Workspace: req.ID,
...
func (h *Handler) handleDeleteWorkspace(c echo.Context) error {
	...
	// 获取工作空间下的所有服务并删除
	services := h.services.GetMcpServices(xl, workspaces.NameArg{
...
func (h *Handler) handleGetWorkspaceServices(c echo.Context) error {
	...
	serviceInfos := []runtime.McpServiceInfo{}
	for _, svc := range services {
		serviceInfos = append(serviceInfos, svc.Info())
	}
