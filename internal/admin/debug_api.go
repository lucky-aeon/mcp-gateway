...
func (h *Handler) handleDebugService(c echo.Context) error {
	...
	// 获取服务实例
	nameArg := workspaces.NameArg{
		Workspace: workspace,
		Server:    serviceName,
	}

	mcpService, err := h.services.GetMcpService(logger, nameArg)
	if err != nil {
	...
	// 检查服务状态
	if serviceInfo.Status != runtime.Running {
...
func (h *Handler) handleGetServiceDebugInfo(c echo.Context) error {
	workspace := c.Param("workspace")
	serviceName := c.Param("name")
...
func (h *Handler) handleTestServiceConnection(c echo.Context) error {
	workspace := c.Param("workspace")
	serviceName := c.Param("name")
	...
	testResult := map[string]interface{}{
		...
		"healthy":      serviceInfo.Status == runtime.Running,
	...
	statusTest := map[string]interface{}{
		...
		"success": serviceInfo.Status == runtime.Running,
	...
	// 测试4: 发送测试消息
	if serviceInfo.Status == runtime.Running && serviceInfo.URLs.MessageUrl != "" {
...
func (h *Handler) handleGetServiceDebugLogs(c echo.Context) error {
	workspace := c.Param("workspace")
	serviceName := c.Param("name")
...
func (h *Handler) setupDebugRoutes(api *echo.Group) {
	// 调试相关路由
	debug := api.Group("/workspaces/:workspace/services/:name/debug")
	debug.GET("/info", h.handleGetServiceDebugInfo)         // 获取调试信息
	debug.POST("/test", h.handleDebugService)               // 发送调试消息
	debug.GET("/connection", h.handleTestServiceConnection) // 测试连接
	debug.GET("/logs", h.handleGetServiceDebugLogs)         // 获取日志

	// API发现和调试路由
	apiDebug := api.Group("/debug")
	apiDebug.GET("/apis", h.handleDiscoverAPIs)        // 获取所有API列表
	apiDebug.POST("/apis/test", h.handleTestAPI)       // 测试API端点
	apiDebug.GET("/apis/groups", h.handleGetAPIGroups) // 获取API分组
}

// handleDiscoverAPIs 自动发现所有API端点
func (h *Handler) handleDiscoverAPIs(c echo.Context) error {
...
// analyzeRoute 分析路由信息
func (h *Handler) analyzeRoute(route *echo.Route) *APIEndpoint {
	// 过滤掉不需要的路由
	...
	// 确定API分组
	group := h.getAPIGroup(route.Path)

	// 分析路径参数
	parameters := h.extractPathParameters(route.Path)

	// 获取处理器名称
	handlerName := h.getHandlerName(route.Name)

	// 添加描述和示例
	description := h.getAPIDescription(route.Method, route.Path, handlerName)
	examples := h.getAPIExamples(route.Method, route.Path)
...
func (h *Handler) getAPIGroup(path string) string {
...
func (h *Handler) extractPathParameters(path string) []APIParameter {
	...
	for _, match := range matches {
		if len(match) > 1 {
			...
			param := APIParameter{
				...
				Location:    "path",
				Required:    true,
				Description: h.getParameterDescription(paramName),
				Example:     h.getParameterExample(paramName),
			}
...
func (h *Handler) getParameterDescription(paramName string) string {
...
func (h *Handler) getParameterExample(paramName string) string {
...
func (h *Handler) getHandlerName(routeName string) string {
	if routeName == "" {
		return "未知处理器"
	}
...
func (h *Handler) getAPIDescription(method, path, handler string) string {
	descriptions := map[string]string{
...
func (h *Handler) getAPIExamples(method, path string) []APIExample {
...
// getAPITags 获取API标签
func (h *Handler) getAPITags(path string) []string {
	var tags []string
...
func (h *Handler) handleGetAPIGroups(c echo.Context) error {
...
func (h *Handler) handleTestAPI(c echo.Context) error {
